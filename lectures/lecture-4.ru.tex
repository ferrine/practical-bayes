...
\begin{frame}{Непараметрические модели}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item Экстраполирует периодически
...
\begin{frame}{Математика ядерных методов}
    Ядра можно комбинировать (\href{\usepackage{hyperref}}{тут подробнее}). Если $k_1(x, x')$ и $k_2(x, x')$ удовлетворяют свойствам ядра, то
    \begin{enumerate}
...
\begin{figure}
    ...
    \caption{Множественные периодические ядра}
\end{figure}
\end{frame}
...
\begin{align*}
    ...
    \only<3>{
    \begin{block}{Комментарий}
    Проблемы можно решить неориентированной параметризацией 
    \end{block}
    }
    \only<4>{
    \begin{block}{Комментарий}
