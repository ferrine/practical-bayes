...
\begin{frame}{Априорные распределения -- это круто}%Priors are Hot}
    ...
    \begin{column}{0.5\linewidth}
            \begin{itemize}
                \item Априорные распределения -- как регуляризация
                \item Мягкое ограничение пространства решений
                \item Помогают внести в модель экспертные знания
            \end{itemize}
        \end{column}
        \begin{column}{0.5\linewidth}
    ...
    \begin{columns}
        \begin{column}{0.6\linewidth}
            \begin{enumerate}
                ...
                \item Выберите $R^2$
                \item Выберите важность признаков
                \item Выберите знак размера эффекта%Decide on effect size sign
                \item Готово
            \end{enumerate}
        \end{column}
        \begin{column}{0.4\linewidth}
        \begin{figure}
            ...
            Вебинар про линейные регрессии %\\ Awesome Linear Regressions
        \end{figure}

        \end{column}
    \end{columns}
    \begin{block}{Ещё больше формул}
        Это недавно разработанное априорное распределение R2D2M2, см. подробности в \cite{aguilar2023intuitive}. Реализация R2D2M2 в PyMC: \cite{pymc-experimental-r2d2m2cp}.
...
\begin{frame}{Кейс: выращиваем зерно как в Ceres}
    \begin{columns}
    \begin{column}{0.5\linewidth}
    Открытые вопросы в выращивании сельхоз культур
    \begin{itemize}
        \item Как выбрать удобрения?
        \item Как выбрать регион?
        \item Как комбинировать регионы и удобрения?
    \end{itemize}
    \begin{block}{Решение}
        Всестороннее А/Б тестирование%Extensive AB testing required
    \end{block}
    \end{column}
    \begin{column}{0.5\linewidth}
